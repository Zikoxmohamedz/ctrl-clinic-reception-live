...
بعد ترقية مشروع قائم، نفّذ [inventory-upgrade.sql](inventory-upgrade.sql) مرة واحدة في Supabase SQL Editor لإضافة جلسات الجرد المشتركة، الصلاحيات، والتحديث اللحظي. ولتفعيل دورة رصيد أول المدة وفروق العجز والزيادة، نفّذ بعدها [inventory-balance-upgrade.sql](inventory-balance-upgrade.sql) مرة واحدة؛ أول تشغيل فقط يثبت الرصيد الحالي كرصيد أول مدة. ثم أعد نشر دالة `invite-user` حتى يحصل الموظفون الجدد على صلاحية الجرد المحددة لهم.
